...
bool charserial::operator==(const charserial &other) const{
    return lowCase(value) == other.lowCase(other.getValue());
}
bool charserial::operator<(const charserial &other) const{
    return lowCase(value) < other.lowCase(other.getValue());
}
...
string charserial::toString(){
    string repr = "";
    repr += string(reinterpret_cast<char*>(&value), sizeof(value));
    return repr;
}
void charserial::fromString(string repr){
    int pos = 0;
    int asciivalue = 0;
    repr.copy(reinterpret_cast<char*>(&asciivalue), sizeof(value), pos);
