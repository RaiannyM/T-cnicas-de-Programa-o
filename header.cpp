...
header::header() : serializable() { // Construtor padrão
    type = "***";
    firstDeleted = 0;
    root = 0;
    version = 0;
...
header::header(const string t, unsigned int v) : serializable() { // Construtor paramétrico
    type = t.substr(0, 3);
    firstDeleted = 0;
    root = 0;
    version = v;
...
header::header(const header &h){ // Construtor de cópia
    type = h.getType();
...
header::~header(){ // Destrutor

}

header header::operator=(const header &h){ // Sobrecarga do operador =
    header aux(h);
    if (this == &h){
        return *this;
    }
...
bool header::operator==(const header &h){ // Sobrecarga do operador ==
